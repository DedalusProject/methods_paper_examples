"""
    NonLinear Schrodinger equation on the Fano plane.

    This is allows arbitary topology with a few caveats:
        The PDE parameters are all set to 1.
        There are no free nodes.
        Boundary conditions are required for these if so.

    This script should be ran serially (because it is 1D), and creates
    a colloection of space-time plots of the computed solution on each edge of the network.
...
#Incidence matrix. This should really use a sparse matrix.
